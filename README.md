...
1. **LMのsoft-hardギャップの原因単離**: v0.3チェックポイントでブランチ別(votes/static)ハード評価 → 崩れている側にone-hot正則化 or 段階的ハード化(層単位のfreeze)
2. LM品質: trigramカウント超え → Transformer(2.68)へ。文脈幅3-gramキー、静的回路のさらなる容量、複数シード
3. GPU環境へ移行(`PORTING.md`)、スケール検証 → ModernBERT蒸留
